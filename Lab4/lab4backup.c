...
int find_next_byte_match(int *position, char *byte, char *mem_start, int mem_size)
{
	...
	int bytenumber = (int)strtol(byte, NULL, 16);
	for (i = *position + 1; i < *mem_start + mem_size; i++) {
		if (*(mem_start + i) == bytenumber) {
			*position = i;
			found = 1;
			break;
		}
...
{
    int found = 0, i, j, temp_pos = 0;
	i = *position + 1;
	j = 0;
	while (i < *mem_start + mem_size) {
		if (*(mem_start + i) == str[j]) {
			i++;
			j++;
			if (temp_pos == 0) {
				temp_pos = i - 1;
...
{
    int found = 0, i;
	int bytenumber = (int)strtol(byte_to_find, NULL, 16);
	for (i = *position + 1; i < *mem_start + mem_size; i++) {
		if (*(mem_start + i) == bytenumber) {
			*position = i;
			found = 1;
	...
	*(mem_start + *position) = int_to_replace;
    return found;
}
/*----------------------------------------------------------*/
...
{
	int i, j, numprint;
	int offset = position - position % 16;
	...
	for (j = 0; j < numprint; j++) {
		...
		if (slen > 1) {
			if (j == 0) {
				for (i = position % 16; i < 16; i++) {
					if (slen == 0) {
						printf("--|\n");
						break;
					}
					if (i == wild_pos + position % 16) {
						printf("*-");
						slen--;
					}
					printf("---");
				...
				for (i = 0; i < 16; i++) {
					...
					if (i == wild_pos - (16 - (position - offset))) {
						printf("*-");
						slen--;
					}
					printf("---");
...
{
    int wild_position = -1, i, j;  // -1 means no wildcard found
...
{
    int wild_position = -1, i = 1, j = 0;  // -1 means no wildcard found
	while (1) {
		if ((inputstr[i] == '.') && (wild_position == -1)) {
			wild_position = i;
		} else if (inputstr[i] == '\\') {
		...
		i++;
		j++;
		if (inputstr[i] == '/' && inputstr[i-1] != '\\' && i != 0) {
			break;
		}
	}
	if (searchstr[j-1] == 0) {
	...
	replacestr[j-1] = '\0';
	if (strlen(replacestr) != strlen(searchstr)) {
		wild_position = -2;
	}
    return wild_position;
}
...
char *store_file(char * filename, int * file_size)
{
    ...
    *file_size = 0;
	...
	fileptr = (char *)malloc(*file_size * sizeof(char));
	if (fileptr ==  NULL) {
